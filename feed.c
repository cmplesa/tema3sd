...
void feed(post_array_t **post_array, int ***relations,
...
node_posts_t *print_node_by_user_id(node_posts_t *root,
									node_posts_t *root_check, int user_id)
{
...
node_posts_t *print_node_by_user_id_repost(node_posts_t *root,
										   int user_id, char *title)
{
...
void print_friends_that_reposted(int ***relations,
								 node_posts_t *root, int user_id, int post_id)
{
	if (check_if_friend(relations, root->user_id, user_id) &&
		get_user_name(root->user_id) != get_user_name(user_id))
		printf("%s\n", get_user_name(root->user_id));
	for (int i = 0; i < root->children_number; i++)
		print_friends_that_reposted(relations,
									root->children[i], user_id, post_id);
}

void see_friends_reposted(int ***relations, post_array_t **post_array,
						  char *name, int post_id)
{
	for (int i = 0; i < (*post_array)->number_of_posts; i++) {
		if ((*post_array)->posts[i]->root->post_id == post_id) {
			node_posts_t *root = (*post_array)->posts[i]->root;
			print_friends_that_reposted(relations, root,
										get_user_id(name), post_id);
			break;
		}
	}
}
